...
@interface SYLameTools ()
@property (nonatomic, assign) BOOL stopRecord;

@end

@implementation SYLameTools

SYSingleCaseM(SYLameTools)
...
-(void)audioRecodingToMP3:(NSString *)sourcePath isDeleteSourchFile: (BOOL)isDelete withSuccessBack:(void(^)(NSString *resultPath))success withFailBack:(void(^)(NSString *error))fail
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        @try {
            ...
            do {
                ...
                if (length > PCM_SIZE * 2 * sizeof(short int)) {
                    ...
                    if (!isSkipPCMHeader) {
                        ...
                        isSkipPCMHeader = YES;
                        NSLog(@"SY skip pcm file header !!!!!!!!!!");
                    }
                    read = (int)fread(pcm_buffer, 2 * sizeof(short int), PCM_SIZE, pcm);
                    write = lame_encode_buffer_interleaved(lame, pcm_buffer, read, mp3_buffer, MP3_SIZE);
                    fwrite(mp3_buffer, write, 1, mp3);
                    NSLog(@"SY read %d bytes", write);
                } else {
            ...
            read = (int)fread(pcm_buffer, 2 * sizeof(short int), PCM_SIZE, pcm);
            write = lame_encode_flush(lame, mp3_buffer, MP3_SIZE);
            NSLog(@"SY read %d bytes and flush to mp3 file", write);
        ...
        @finally {
            // NSLog(@"SY convert mp3 finish!!! %@", outPath);
            if (isDelete) {
                NSError *error;
                [fm removeItemAtPath:sourcePath error:&error];
                if (error == nil){
