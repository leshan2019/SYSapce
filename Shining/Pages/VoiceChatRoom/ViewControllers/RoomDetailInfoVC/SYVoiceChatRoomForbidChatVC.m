...
#define SYVoiceChatRoomForBidChatCellID @"SYVoiceChatRoomForBidChatCellID"

@interface SYVoiceChatRoomForbidChatVC ()<SYCommonTopNavigationBarDelegate, UITableViewDelegate, UITableViewDataSource, SYNoNetworkViewDelegate>

@property (nonatomic, strong) SYCommonTopNavigationBar *topNavBar;
@property (nonatomic, strong) UITableView *listView;
@property (nonatomic, strong) SYVoiceChatRoomForbidChatViewModel *viewModel;
...
@implementation SYVoiceChatRoomForbidChatVC
...
- (void)requestFirstPageData {
    ...
    __weak typeof(self) weakSelf = self;
    [self.viewModel requestForbidChatUserListWithChannelID:self.channelId page:1 success:^(BOOL success, NSInteger pageSize) {
...
- (void)requestMoreData {
    __weak typeof(self) weakSelf = self;
    [self.viewModel requestMoreForbidChatUserListWithChannelId:self.channelId success:^(BOOL success, NSInteger pageSize) {
        weakSelf.isLoadMoreData = NO;
        [weakSelf.listView.mj_footer endRefreshing];
        if (success) {
            [weakSelf.listView reloadData];
...
// 删除禁言人
- (void)deleteForbidUserChatWithChannelId:(NSString *)channelId withUid:(NSString *)uid withIndexPath:(NSIndexPath *)indexPath {
    __weak typeof(self) weakSelf = self;
    [self.viewModel requestCancelForbidUserChatWithChannelID:channelId withUid:uid withIndexPath:indexPath success:^(BOOL success) {
        if (success) {
            [weakSelf.listView reloadData];
            [SYToastView showToast:@"删除成功"];
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(voiceRoomInfoDidRemoveForbiddenChatUser:)]) {
                [weakSelf.delegate voiceRoomInfoDidRemoveForbiddenChatUser:uid];
            }
        } else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SYVoiceChatRoomManagerCell *cell = (SYVoiceChatRoomManagerCell *)[tableView dequeueReusableCellWithIdentifier:SYVoiceChatRoomForBidChatCellID];
    if (!cell) {
        cell = [[SYVoiceChatRoomManagerCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:SYVoiceChatRoomForBidChatCellID];
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        NSString *userId = [self.viewModel userIdWithIndexPath:indexPath];
        [self deleteForbidUserChatWithChannelId:self.channelId withUid:userId withIndexPath:indexPath];
    }
}
...
- (SYCommonTopNavigationBar *)topNavBar {
    if (!_topNavBar) {
        _topNavBar = [[SYCommonTopNavigationBar alloc] initWithFrame:CGRectZero midTitle:@"禁言名单" rightTitle:@"" hasAddBtn:NO];
...
- (SYVoiceChatRoomForbidChatViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [SYVoiceChatRoomForbidChatViewModel new];
