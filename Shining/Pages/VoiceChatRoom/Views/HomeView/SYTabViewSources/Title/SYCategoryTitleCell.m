...
#import "SYCategoryTitleCell.h"
#import "SYCategoryTitleCellModel.h"
#import "SYCategoryFactory.h"

@interface SYCategoryTitleCell ()
...
@implementation SYCategoryTitleCell
...
- (void)layoutSubviews {
    ...
    //因为titleLabel是通过约束布局的，在layoutSubviews方法中，它的frame并没有确定。像子类SYCategoryNumberCell中的numberLabel需要依赖于titleLabel的frame进行布局。所以这里必须立马触发self.contentView的视图布局。
    [self.contentView setNeedsLayout];
    [self.contentView layoutIfNeeded];
    SYCategoryTitleCellModel *myCellModel = (SYCategoryTitleCellModel *)self.cellModel;
    switch (myCellModel.titleLabelAnchorPointStyle) {
        case SYCategoryTitleLabelAnchorPointStyleCenter:
            self.titleLabelCenterY.constant = 0 + myCellModel.titleLabelVerticalOffset;
            break;
        case SYCategoryTitleLabelAnchorPointStyleTop:
        {
            ...
            break;
        case SYCategoryTitleLabelAnchorPointStyleBottom:
        {
            CGFloat percent = (myCellModel.titleLabelCurrentZoomScale - myCellModel.titleLabelNormalZoomScale)/(myCellModel.titleLabelSelectedZoomScale - myCellModel.titleLabelNormalZoomScale);
            self.titleLabelCenterY.constant = self.titleLabel.bounds.size.height/2 + myCellModel.titleLabelVerticalOffset + myCellModel.titleLabelZoomSelectedVerticalOffset*percent;
        }
...
- (void)reloadData:(SYCategoryBaseCellModel *)cellModel {
    ...
    SYCategoryTitleCellModel *myCellModel = (SYCategoryTitleCellModel *)cellModel;
    ...
    if (myCellModel.isTitleLabelZoomEnabled) {
        ...
        if (myCellModel.isSelectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            SYCategoryCellSelectedAnimationBlock block = [self preferredTitleZoomAnimationBlock:myCellModel baseScale:baseScale];
    ...
    if (myCellModel.isTitleLabelStrokeWidthEnabled) {
        if (myCellModel.isSelectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            SYCategoryCellSelectedAnimationBlock block = [self preferredTitleStrokeWidthAnimationBlock:myCellModel attributedString:attributedString];
            [self addSelectedAnimationBlock:block];
        }else {
    ...
    if (myCellModel.isTitleLabelMaskEnabled) {
        ...
        self.titleLabel.layer.mask = nil;
        if (myCellModel.isSelectedAnimationEnabled && [self checkCanStartSelectedAnimation:myCellModel]) {
            SYCategoryCellSelectedAnimationBlock block = [self preferredTitleColorAnimationBlock:myCellModel];
            [self addSelectedAnimationBlock:block];
        }else {
...
- (SYCategoryCellSelectedAnimationBlock)preferredTitleZoomAnimationBlock:(SYCategoryTitleCellModel *)cellModel baseScale:(CGFloat)baseScale {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            //将要选中，scale从小到大插值渐变
            cellModel.titleLabelCurrentZoomScale = [SYCategoryFactory interpolationFrom:cellModel.titleLabelNormalZoomScale to:cellModel.titleLabelSelectedZoomScale percent:percent];
        }else {
            //将要取消选中，scale从大到小插值渐变
            cellModel.titleLabelCurrentZoomScale = [SYCategoryFactory interpolationFrom:cellModel.titleLabelSelectedZoomScale to:cellModel.titleLabelNormalZoomScale percent:percent];
        }
...
- (SYCategoryCellSelectedAnimationBlock)preferredTitleStrokeWidthAnimationBlock:(SYCategoryTitleCellModel *)cellModel attributedString:(NSMutableAttributedString *)attributedString {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleLabelCurrentStrokeWidth = [SYCategoryFactory interpolationFrom:cellModel.titleLabelNormalStrokeWidth to:cellModel.titleLabelSelectedStrokeWidth percent:percent];
        }else {
            //将要取消选中，StrokeWidth从大到小插值渐变
            cellModel.titleLabelCurrentStrokeWidth = [SYCategoryFactory interpolationFrom:cellModel.titleLabelSelectedStrokeWidth to:cellModel.titleLabelNormalStrokeWidth percent:percent];
        }
...
- (SYCategoryCellSelectedAnimationBlock)preferredTitleColorAnimationBlock:(SYCategoryTitleCellModel *)cellModel {
    ...
    return ^(CGFloat percent) {
        if (cellModel.isSelected) {
            ...
            cellModel.titleCurrentColor = [SYCategoryFactory interpolationColorFrom:cellModel.titleNormalColor to:cellModel.titleSelectedColor percent:percent];
        }else {
            //将要取消选中，textColor从titleSelectedColor到titleNormalColor插值渐变
            cellModel.titleCurrentColor = [SYCategoryFactory interpolationColorFrom:cellModel.titleSelectedColor to:cellModel.titleNormalColor percent:percent];
        }
