...
@class SYCategoryTitleView;

@protocol SYCategoryTitleViewDataSource <NSObject>
@optional
// 如果将SYCategoryTitleView嵌套进UITableView的cell，每次重用的时候，SYCategoryTitleView进行reloadData时，会重新计算所有的title宽度。所以该应用场景，需要UITableView的cellModel缓存titles的文字宽度，再通过该代理方法返回给SYCategoryTitleView。
// 如果实现了该方法就以该方法返回的宽度为准，不触发内部默认的文字宽度计算。
- (CGFloat)categoryTitleView:(SYCategoryTitleView *)titleView widthForTitle:(NSString *)title;
@end


@interface SYCategoryTitleView : SYCategoryIndicatorView

@property (nonatomic, weak) id<SYCategoryTitleViewDataSource> titleDataSource;
...
@property (nonatomic, assign) SYCategoryTitleLabelAnchorPointStyle titleLabelAnchorPointStyle;  //titleLabel锚点位置，用于调整titleLabel缩放时的基准位置。默认为：SYCategoryTitleLabelAnchorPointStyleCenter
@end
