...
@interface SYCategoryIndicatorComponentView : UIView <SYCategoryIndicatorProtocol>

/**
 指示器的位置。底部或者顶部
 */
@property (nonatomic, assign) SYCategoryComponentPosition componentPosition;

/**
 默认SYCategoryViewAutomaticDimension（与cell的宽度相等）。内部通过`- (CGFloat)indicatorWidthValue:(CGRect)cellFrame`方法获取实际的值
 */
...
/**
 默认SYCategoryViewAutomaticDimension （等于indicatorHeight/2）。内部通过`- (CGFloat)indicatorCornerRadiusValue:(CGRect)cellFrame`方法获取实际的值
 */
...
/**
 手势滚动、点击切换的时候，如果允许滚动，分为简单滚动和复杂滚动。默认为：SYCategoryIndicatorScrollStyleSimple
 目前仅SYCategoryIndicatorLineView、SYCategoryIndicatorDotLineView支持，其他子类暂不支持。
 */
@property (nonatomic, assign) SYCategoryIndicatorScrollStyle scrollStyle;
