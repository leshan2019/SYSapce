...
#import "SYCategoryListContainerView.h"
#import <objc/runtime.h>

@interface SYCategoryListContainerViewController : UIViewController
...
@implementation SYCategoryListContainerViewController
- (void)dealloc
{
...
@interface SYCategoryListContainerView () <UIScrollViewDelegate, UICollectionViewDelegate, UICollectionViewDataSource>
@property (nonatomic, weak) id<SYCategoryListContainerViewDelegate> delegate;
@property (nonatomic, strong) UIScrollView *scrollView;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, id<SYCategoryListContentViewDelegate>> *validListDict;
@property (nonatomic, assign) NSInteger willAppearIndex;
@property (nonatomic, assign) NSInteger willDisappearIndex;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) SYCategoryListContainerViewController *containerVC;
@end

@implementation SYCategoryListContainerView

- (instancetype)initWithType:(SYCategoryListContainerType)type delegate:(id<SYCategoryListContainerViewDelegate>)delegate{
...
- (void)initializeViews {
    _containerVC = [[SYCategoryListContainerViewController alloc] init];
    ...
    if (self.containerType == SYCategoryListContainerType_ScrollView) {
        if (self.delegate &&
...
- (void)layoutSubviews {
    ...
    if (self.containerType == SYCategoryListContainerType_ScrollView) {
        if (CGRectEqualToRect(self.scrollView.frame, CGRectZero) ||  !CGSizeEqualToSize(self.scrollView.bounds.size, self.bounds.size)) {
            ...
            [_validListDict enumerateKeysAndObjectsUsingBlock:^(NSNumber * _Nonnull index, id<SYCategoryListContentViewDelegate>  _Nonnull list, BOOL * _Nonnull stop) {
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    id<SYCategoryListContentViewDelegate> list = _validListDict[@(indexPath.item)];
...
#pragma mark - SYCategoryViewListContainer
...
- (void)reloadData {
    for (id<SYCategoryListContentViewDelegate> list in _validListDict.allValues) {
...
- (void)initListIfNeededAtIndex:(NSInteger)index {
    ...
    id<SYCategoryListContentViewDelegate> list = _validListDict[@(index)];
    if (list != nil) {
        //列表已经创建好了
        return;
    }
