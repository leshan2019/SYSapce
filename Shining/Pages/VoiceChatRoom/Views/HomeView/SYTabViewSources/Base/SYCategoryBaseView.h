...
@class SYCategoryBaseView;

@protocol SYCategoryViewListContainer <NSObject>
...
@protocol SYCategoryViewDelegate <NSObject>
...
/**
 ...
 @param index 选中的index
 */
- (void)categoryView:(SYCategoryBaseView *)categoryView didClickSelectedItemAtIndex:(NSInteger)index;
...
/**
 ...
 @param index 选中的index
 */
- (void)categoryView:(SYCategoryBaseView *)categoryView didScrollSelectedItemAtIndex:(NSInteger)index;
...
/**
 ...
 @return 能否点击
 */
- (BOOL)categoryView:(SYCategoryBaseView *)categoryView canClickItemAtIndex:(NSInteger)index;
 ...
 */
- (void)categoryView:(SYCategoryBaseView *)categoryView scrollingFromLeftIndex:(NSInteger)leftIndex toRightIndex:(NSInteger)rightIndex ratio:(CGFloat)ratio;

@end

@interface SYCategoryBaseView : UIView

@property (nonatomic, strong, readonly) SYCategoryCollectionView *collectionView;

@property (nonatomic, strong) NSArray <SYCategoryBaseCellModel *> *dataSource;

@property (nonatomic, weak) id<SYCategoryViewDelegate> delegate;

/**
 高封装度的列表容器，使用该类可以让列表拥有完成的生命周期、自动同步defaultSelectedIndex、自动调用reloadData。
 */
@property (nonatomic, weak) id<SYCategoryViewListContainer> listContainer;
...
@property (nonatomic, assign) CGFloat contentEdgeInsetLeft;     //整体内容的左边距，默认SYCategoryViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat contentEdgeInsetRight;    //整体内容的右边距，默认SYCategoryViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat cellWidth;    //默认SYCategoryViewAutomaticDimension
...
@property (nonatomic, assign, getter=isAverageCellSpacingEnabled) BOOL averageCellSpacingEnabled;     //当collectionView.contentSize.width小于SYCategoryBaseView的宽度，是否将cellSpacing均分。默认为YES。
...
/**
 刷新指定的index的cell
 内部会触发`- (void)refreshCellModel:(SYCategoryBaseCellModel *)cellModel index:(NSInteger)index`方法进行cellModel刷新
...
@interface SYCategoryBaseView (UISubclassingBaseHooks)
 ...
 */
- (void)refreshSelectedCellModel:(SYCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(SYCategoryBaseCellModel *)unselectedCellModel NS_REQUIRES_SUPER;
...
/**
 ...
 @param index 选中的index
 @param selectedType SYCategoryCellSelectedType
 @return 返回值为NO，表示触发内部某些判断（点击了同一个cell），子类无需后续操作。
 */
- (BOOL)selectCellAtIndex:(NSInteger)index selectedType:(SYCategoryCellSelectedType)selectedType NS_REQUIRES_SUPER;
 ...
 */
- (void)refreshCellModel:(SYCategoryBaseCellModel *)cellModel index:(NSInteger)index NS_REQUIRES_SUPER;
