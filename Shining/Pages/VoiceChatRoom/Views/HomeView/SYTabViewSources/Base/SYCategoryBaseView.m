...
@interface SYCategoryBaseView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) SYCategoryCollectionView *collectionView;
...
@implementation SYCategoryBaseView
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    SYCategoryBaseCellModel *cellModel = self.dataSource[index];
    cellModel.selectedType = SYCategoryCellSelectedTypeUnknown;
    [self refreshCellModel:cellModel index:index];
    SYCategoryBaseCell *cell = (SYCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    [cell reloadData:cellModel];
}

- (void)selectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:SYCategoryCellSelectedTypeCode];
}
...
- (void)layoutSubviews
{
    ...
    //部分使用者为了适配不同的手机屏幕尺寸，SYCategoryView的宽高比要求保持一样，所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。所以，为了规避这个问题，在这里对高度统一向下取整。
    //如果向下取整导致了你的页面异常，请自己重新设置SYCategoryView的高度，保证为整数即可。
...
- (void)setDelegate:(id<SYCategoryViewDelegate>)delegate {
    _delegate = delegate;
...
- (void)setListContainer:(id<SYCategoryViewListContainer>)listContainer {
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    SYCategoryBaseCellModel *cellModel = self.dataSource[indexPath.item];
    cellModel.selectedType = SYCategoryCellSelectedTypeUnknown;
    [(SYCategoryBaseCell *)cell reloadData:cellModel];
}

- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    BOOL isTransitionAnimating = NO;
    for (SYCategoryBaseCellModel *cellModel in self.dataSource) {
...
- (CGFloat)getContentEdgeInsetLeft {
    if (self.contentEdgeInsetLeft == SYCategoryViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (CGFloat)getContentEdgeInsetRight {
    if (self.contentEdgeInsetRight == SYCategoryViewAutomaticDimension) {
        return self.innerCellSpacing;
    }
...
- (void)clickSelectItemAtIndex:(NSInteger)index {
    ...
    [self selectCellAtIndex:index selectedType:SYCategoryCellSelectedTypeClick];
}

- (void)scrollSelectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:SYCategoryCellSelectedTypeScroll];
}
...
@implementation SYCategoryBaseView (UISubclassingBaseHooks)

- (CGRect)getTargetCellFrame:(NSInteger)targetIndex
{
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i ++) {
        SYCategoryBaseCellModel *cellModel = self.dataSource[i];
        CGFloat cellWidth;
    ...
    CGFloat width;
    SYCategoryBaseCellModel *selectedCellModel = self.dataSource[targetIndex];
...
- (CGRect)getTargetSelectedCellFrame:(NSInteger)targetIndex selectedType:(SYCategoryCellSelectedType)selectedType
{
    CGFloat x = [self getContentEdgeInsetLeft];
    for (int i = 0; i < targetIndex; i ++) {
        SYCategoryBaseCellModel *cellModel = self.dataSource[i];
...
- (void)initializeData
{
    ...
    _selectedIndex = 0;
    _cellWidth = SYCategoryViewAutomaticDimension;
    ...
    _contentEdgeInsetLeft = SYCategoryViewAutomaticDimension;
    _contentEdgeInsetRight = SYCategoryViewAutomaticDimension;
...
- (void)initializeViews
{
    ...
    _collectionView = [[SYCategoryCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)refreshState {
    ...
    if (self.isAverageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(SYCategoryBaseCellModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(SYCategoryBaseCellModel * cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (CGRectEqualToRect(self.contentScrollView.frame, CGRectZero) && self.contentScrollView.superview != nil) {
        //某些情况系统会出现SYCategoryView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现contentScrollView的frame为zero时，强行触发其父视图链里面已经有frame的一个父视图的layoutSubviews方法。
        //比如SYSegmentedListContainerView会将contentScrollView包裹起来使用，该情况需要SYSegmentedListContainerView.superView触发布局更新
...
- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(SYCategoryCellSelectedType)selectedType {
    ...
    if (self.selectedIndex == targetIndex) {
        ...
        if (selectedType == SYCategoryCellSelectedTypeCode) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
        }else if (selectedType == SYCategoryCellSelectedTypeClick) {
            [self.listContainer didClickSelectedItemAtIndex:targetIndex];
        ...
        }else if (selectedType == SYCategoryCellSelectedTypeScroll) {
    ...
    SYCategoryBaseCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    ...
    //刷新当前选中的和将要选中的cell
    SYCategoryBaseCell *lastCell = (SYCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    SYCategoryBaseCell *selectedCell = (SYCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
    [selectedCell reloadData:selectedCellModel];

    if (self.scrollingTargetIndex != -1 && self.scrollingTargetIndex != targetIndex) {
        SYCategoryBaseCellModel *scrollingTargetCellModel = self.dataSource[self.scrollingTargetIndex];
        ...
        SYCategoryBaseCell *scrollingTargetCell = (SYCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.scrollingTargetIndex inSection:0]];
    ...
    if (selectedType == SYCategoryCellSelectedTypeClick ||
        selectedType == SYCategoryCellSelectedTypeCode) {
...
- (void)refreshSelectedCellModel:(SYCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(SYCategoryBaseCellModel *)unselectedCellModel {
    ...
    if (self.isCellWidthZoomEnabled) {
        if (selectedCellModel.selectedType == SYCategoryCellSelectedTypeCode ||
            selectedCellModel.selectedType == SYCategoryCellSelectedTypeClick) {
            ...
            self.animator.progressCallback = ^(CGFloat percent) {
                ...
                selectedCellModel.cellWidthCurrentZoomScale = [SYCategoryFactory interpolationFrom:selectedCellModel.cellWidthNormalZoomScale to:selectedCellModel.cellWidthSelectedZoomScale percent:percent];
                ...
                unselectedCellModel.cellWidthCurrentZoomScale = [SYCategoryFactory interpolationFrom:unselectedCellModel.cellWidthSelectedZoomScale to:unselectedCellModel.cellWidthNormalZoomScale percent:percent];
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (self.animator.isExecuting) {
            ...
            for (SYCategoryBaseCellModel *model in self.dataSource) {
                if (model.isSelected) {
        ...
        if (self.isCellWidthZoomEnabled && self.isCellWidthZoomScrollGradientEnabled) {
            SYCategoryBaseCellModel *leftCellModel = (SYCategoryBaseCellModel *)self.dataSource[baseIndex];
            SYCategoryBaseCellModel *rightCellModel = (SYCategoryBaseCellModel *)self.dataSource[baseIndex + 1];
            leftCellModel.cellWidthCurrentZoomScale = [SYCategoryFactory interpolationFrom:leftCellModel.cellWidthSelectedZoomScale to:leftCellModel.cellWidthNormalZoomScale percent:remainderRatio];
            leftCellModel.cellWidth = [self getCellWidthAtIndex:leftCellModel.index] * leftCellModel.cellWidthCurrentZoomScale;
            rightCellModel.cellWidthCurrentZoomScale = [SYCategoryFactory interpolationFrom:rightCellModel.cellWidthNormalZoomScale to:rightCellModel.cellWidthSelectedZoomScale percent:remainderRatio];
...
- (Class)preferredCellClass {
    return SYCategoryBaseCell.class;
}
