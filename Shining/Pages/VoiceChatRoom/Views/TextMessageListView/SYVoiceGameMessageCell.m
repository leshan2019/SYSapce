...
@interface SYVoiceGameMessageCell ()
...
@implementation SYVoiceGameMessageCell
...
- (void)showWithViewModel:(SYVoiceTextMessageViewModel *)viewModel {
    ...
    CGFloat textMaxWidth = (self.sy_width - 105.f - 80.f);
    if (isBroadcaster) {
        textMaxWidth -= 38.f;
    }
    if (iPhone5) {
...
- (UILabel *)senderLabel {
    if (!_senderLabel) {
        _senderLabel = [[UILabel alloc] initWithFrame:CGRectMake(20.f, 6.f, 0, 20.f)];
        _senderLabel.font = [UIFont systemFontOfSize:14.f weight:UIFontWeightSemibold];
