//
//  SYVoiceChatRoomForbidEnterViewModel.m
//  Shining
//
...
@interface SYVoiceChatRoomForbidEnterViewModel ()
...
@implementation SYVoiceChatRoomForbidEnterViewModel
...
- (void)requestForbidEnterUserListWithChannelID:(NSString *)channelID page:(NSInteger)page success:(GetForbidEnterListSuccess)success {
    page = 1;
    self.currentPage = 1;
    self.totalPage = 1;
    __weak typeof(self)weakSelf = self;
    [self.netManager requestForbidEnterUserListWithChannelID:channelID page:page success:^(id  _Nullable response) {
...
- (void)requestMoreForbidEnterUserListWithChannelId:(NSString *)channelId success:(GetForbidEnterListSuccess)success {
    if (self.currentPage >= self.totalPage) {
        success(NO, 0);
    } else {
        self.currentPage ++;
        __weak typeof(self)weakSelf = self;
        [self.netManager requestForbidEnterUserListWithChannelID:channelId page:self.currentPage success:^(id  _Nullable response) {
            if ([response isKindOfClass:[SYVoiceRoomUserListModel class]]) {
                SYVoiceRoomUserListModel *model = (SYVoiceRoomUserListModel *)response;
...
- (void)requestCancelForbidUserEnterWithChannelID:(NSString *)channelID withUid:(NSString *)uid withIndexPath:(NSIndexPath *)indexPath success:(DeleteForbidEnterListSuccess)success {
    __weak typeof(self)weakSelf = self;
    [self.netManager requestCancelForbidUserEnterWithChannelID:channelID uid:uid success:^(id  _Nullable response) {
