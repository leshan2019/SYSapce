//
//  SYMyCoinDetailViewController.m
//  Shining
//
...
#import "MyCoinDetailTableViewCell.h"
#import "SYCoinDetailModel.h"
//#import "MJRefresh.h"

@interface SYMyCoinDetailViewController ()<MyShineDetailTopBarViewDelegate,UITableViewDelegate,UITableViewDataSource,
SYCommonTopNavigationBarDelegate>

@property (nonatomic, strong) MyShineDetailTopBarView *topBarView;//顶部栏
@property (nonatomic, strong) UITableView *tableView;

@property (nonatomic, strong) UITableView *consumeTableView;

@property (nonatomic, strong) SYCoinDetailViewModel *viewModel;
...
@implementation SYMyCoinDetailViewController
...
- (void)viewDidLoad {
    ...
    [self setUpView];
    [self sy_configDataInfoPageName:SYPageNameType_Wallet_Detailed];
    self.showRefreshHeader = YES;
    self.showConsumeRefreshHeader = YES;
    if (!_viewModel) {
        _viewModel = [[SYCoinDetailViewModel alloc] init];
    }
...
- (SYCommonTopNavigationBar *)topNavBar {
    if (!_topNavBar) {
        _topNavBar = [[SYCommonTopNavigationBar alloc] initWithFrame:CGRectZero midTitle:@"蜜豆明细" rightTitle:@"" hasAddBtn:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MyCoinDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MyCoinDetailTableViewCell"];
    if (!cell) {
        cell = [[MyCoinDetailTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MyCoinDetailTableViewCell"];
    }
    if (self.topBarView.selectedControl == 0) {
        if (![self.viewModel isIncomeEmptyData]) {
            SYCoinDetailModel *model = [self.viewModel.incomeListModel.list objectAtIndex:indexPath.row];
            [cell bindData:model];
        }
        return cell;
    }else{
        if (![self.viewModel isConsumeEmptyData]) {
            SYCoinDetailModel *model = [self.viewModel.consumeListModel.list objectAtIndex:indexPath.row];
            [cell bindData:model];
        }
        return cell;
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (self.topBarView.selectedControl == 0) {
        if ([self.viewModel isIncomeEmptyData]) {
            ...
            return self.viewModel.incomeListModel.list.count;
        }
    }else{
        if ([self.viewModel isConsumeEmptyData]) {
            return 0;
...
- (void)tableViewDidTriggerHeaderRefresh {
    if (self.topBarView.selectedControl == 0) {
        [self.viewModel getCoinIncomeList:^(BOOL isSuccess) {
            if (isSuccess) {
                if (![self.viewModel isIncomeEmptyData]) {
                    self.emptyView.hidden = YES;
                    self.tableView.hidden = NO;
        ...
        [self.viewModel getCoinConsumeList:^(BOOL isSuccess) {
            if (isSuccess) {
                if (![self.viewModel isConsumeEmptyData]) {
                    self.emptyView.hidden = YES;
                    self.consumeTableView.hidden = NO;
