...
@interface MyCoinDetailTableViewCell ()
...
@implementation MyCoinDetailTableViewCell
...
//        [idTextAttrStr addAttribute:NSKernAttributeName value:@(4)
//                              range:NSMakeRange(0, 2)];

        self.idLbl.attributedText = idTextAttrStr;
        self.idLbl.hidden = NO;
    }
    NSMutableAttributedString *textAttrStr = [[NSMutableAttributedString alloc] init];
    //第一张图
    NSMutableAttributedString * attrStr = [[NSMutableAttributedString alloc] initWithString:item.coins];
    ...
    UIImage* iimg = [UIImage imageNamed_sy:@"voiceroom_note"];
    attach.image = iimg;
    attach.bounds = CGRectMake(0, -4 , iimg.size.width, iimg.size.height);
